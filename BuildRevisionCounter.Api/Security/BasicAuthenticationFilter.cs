...
namespace BuildRevisionCounter.Api.Security
{
    public class BasicAuthenticationFilter : IAuthenticationFilter
    {
        ...
        /// Конструктор фильтра.
        /// </summary>
        /// <param name="userRepository">Объект для получения данных из БД Монго.</param>
        ...
        private async Task<IPrincipal> Authenticate(string userName, string password)
        {
            ...
            var user = await _userRepository.Get(Builders<UserModel>.Filter.Where(u => u.Name == userName));
